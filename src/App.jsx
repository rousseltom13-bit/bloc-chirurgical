...
function mkHeader() {
  ...
  return new Header({ children: [new Table({ width: { size: 9204, type: WidthType.DXA }, columnWidths: [4602, 4602], borders: anb(), rows: [new TableRow({ children: [new TableCell({ borders: anb(), width: { size: 4602, type: WidthType.DXA }, children: [new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 0 }, children: [new ImageRun({ data: logo, transformation: { width: 80, height: 49 }, type: "png" })] })] }), new TableCell({ borders: anb(), width: { size: 4602, type: WidthType.DXA }, children: [new Paragraph({ alignment: AlignmentType.RIGHT, spacing: { after: 10 }, children: [tx("N° FINESS", { size: 14, color: "444444" })] }), new Paragraph({ alignment: AlignmentType.RIGHT, spacing: { after: 0 }, children: [tx("590796975", { size: 16, bold: true })] })] })] })] })] });
}
...
// ─── CONTENT BUILDERS ─────────────────────────────────────────
function crh_pth_paras(civ, nom, prenom, age, dateOp, cote, ind, atcd) {
  return [
...
function crh_ptg_paras(civ, nom, prenom, age, dateOp, cote, ind, def, deg, atcd) {
  return [
...
function crh_lca_paras(civ, nom, prenom, age, dateOp, cote, atcd, men, hasSut) {
  return [
...
async function generateDocs(inter, f, docList) {
  ...
  if (inter === "PTH") {
    ...
    if (docList.includes("CRH")) res["CRH"] = await mkCRH({ nom, prenom, ddn, de: dateOp, ds: "[DATE SORTIE]", dateLettre: dateOp, mt, paras: crh_pth_paras(civ, nom, prenom, age, dateOp, cote, ind, atcd) });
  ...
  if (inter === "PTG") {
    ...
    if (docList.includes("CRH")) res["CRH"] = await mkCRH({ nom, prenom, ddn, de: dateOp, ds: "[DATE SORTIE]", dateLettre: dateOp, mt, paras: crh_ptg_paras(civ, nom, prenom, age, dateOp, cote, ind, def, deg, atcd) });
  ...
  if (inter === "LCA") {
    ...
    if (docList.includes("CRH")) res["CRH"] = await mkCRH({ nom, prenom, ddn, de: "", ds: "", dateLettre: dateOp, mt, paras: crh_lca_paras(civ, nom, prenom, age, dateOp, cote, atcd, men, hasSut) });
