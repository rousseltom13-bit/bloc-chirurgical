...
function crh_pth_paras(civ, nom, prenom, age, dateOp, dateSortie, cote, ind, atcd, typeAnesthesie) {
  return [
    ...
    { texte: "L'intervention s'est parfaitement déroulée au bloc opératoire le " + dateOp + " sous anesthésie " + f.typeAnesthesie + ". Les radiographies de contrôle post-opératoire sont satisfaisantes.", after: 120 },
...
async function generateDocs(inter, f, docList) {
  ...
  if (inter === "PTH") {
    ...
    if (docList.includes("CRH")) res["CRH"] = await mkCRH({ nom, prenom, ddn, de: dateOp, ds: f.dateSortie||"[DATE SORTIE]", dateLettre: f.dateSortie||dateOp, mt, paras: crh_pth_paras(civ, nom, prenom, age, dateOp, f.dateSortie||"[DATE SORTIE]", cote, ind, atcd, f.typeAnesthesie) });
  ...
  if (inter === "PTG") {
    ...
    if (docList.includes("CRH")) res["CRH"] = await mkCRH({ nom, prenom, ddn, de: dateOp, ds: f.dateSortie||"[DATE SORTIE]", dateLettre: f.dateSortie||dateOp, mt, paras: crh_ptg_paras(civ, nom, prenom, age, dateOp, f.dateSortie||"[DATE SORTIE]", cote, ind, def, deg, atcd, f.typeAnesthesie) });
  ...
  if (inter === "LCA") {
    ...
    if (docList.includes("CRO")) res["CRO"] = await mkCRO({ nom, prenom, ddn, dateOp, de: "", ds: "", aides, indication: "Reconstruction du ligament croisé antérieur du genou " + cote + " par technique DT3+2. Ressaut rotatoire " + ressaut + " en pré-opératoire.", ccam: "NFMC003", tempsOp: ["Installation en décubitus dorsal, genou fléchi à 90°.",
          ...
          "Pansement sec."] });
    if (docList.includes("CRH")) res["CRH"] = await mkCRH({ nom, prenom, ddn, de: "", ds: f.dateSortie||"", dateLettre: f.dateSortie||dateOp, mt, paras: crh_lca_paras(civ, nom, prenom, age, dateOp, cote, atcd, men, hasSut, f.typeAnesthesie) });
